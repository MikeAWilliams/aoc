...
fn score_elf_par(
    work_indicies: &Vec<WorkIndices>,
    predicate: fn(&WorkIndices, &WorkIndices) -> bool,
) -> i32 {
...
fn solve_problem_one(input: &Vec<Vec<WorkIndices>>) -> i32 {
    input
        .iter()
        .map(|elf_pair| score_elf_par(elf_pair, elf_pair_is_redundant))
        .sum()
}
...
fn solve_problem_two(input: &Vec<Vec<WorkIndices>>) -> i32 {
    input
        .iter()
        .map(|elf_pair| score_elf_par(elf_pair, elf_pair_overlaps))
        .sum()
}
